...
Figure ~\ref{fig:reg_compareMethods} compares three regression methods with $\alpha = 0.25$ for the gradient descent and $\lambda = 100$ for ridge regression. We see that the three methods give equivalent results. This is surely the consequence of preprocessing the data with PCA, so that the input matrix is not ill conditioned.

\begin{figure}[!h] % !t
	\center
	\includegraphics[width=4in]{figures/comparisonReg.png}
